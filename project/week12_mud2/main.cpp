...
void increaseItemCnt(User& user);
...
int main() {
	...
	int user_y = 0; // 세로 번호
    int count = 0;	// 턴 계산
    Magician user1_magician{}; //유저의 객체 생성(체력 = 20 시작)
	Warrior user2_warrior{}; //유저의 객체 생성(체력 = 20 시작)
	...
	while (1) { // 사용자에게 계속 입력받기 위해 무한 루프
		...
		if (user_input == "상") {
			// 위로 한 칸 올라가기
			user.user_y -= 1;
			if (checkXY(user.user_x, mapX, user.user_y, mapY, user)) {
			...
			// TODO: 아래로 한 칸 내려가기
			user.user_y += 1;
			if (checkXY(user_x, mapX, user_y, mapY, user)) {
